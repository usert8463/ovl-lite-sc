...
async function ytdl(videoUrl, type = 'mp3') {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      const response = await axios.post('https://s47.notube.lol/recover_weight.php', {
        url: videoUrl,
        format: type,
        lang: 'fr',
        subscribed: false
      ...
      const downloadPage = await axios.get(`https://notube.lol/fr/download?token=${token}`);
      const $ = cheerio.load(downloadPage.data);
      const downloadLink = $('#downloadButton').attr('href');
      if (!downloadLink) throw new Error('Lien de téléchargement non trouvé');

      return downloadLink;
    } catch (e) {
      console.warn(`Tentative ${attempt} échouée : ${e.message}`);
