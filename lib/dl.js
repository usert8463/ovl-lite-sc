      ...
      const sessionCookies = Object.entries({
        ...
        .join("; ");
            ...
            const { data: html } = await axios.post(
                ...
                { id: videoUrl, locale: "fr", tt: "ZzdFcGI2" },
                { headers: { "Content-Type": "application/json", "cookie": sessionCookies, "User-Agent": "GoogleBot" } }
            );

            const $ = cheerio.load(html);
            const hdVideo = $("#hd_download").attr("data-directurl") ? "https://ssstik.io" + $("#hd_download").attr("data-directurl") : null;
            const noWatermark = $("a.download_link.without_watermark").attr("href") || null;
            const mp3 = $("a.download_link.music").attr("href") || null;
            const slides = [];
            $("a.download_link.slide").each((_, el) => slides.push($(el).attr("href")));

            return { hdVideo, noWatermark, mp3, slides };
        } catch (err) {
            lastError = err;
            tries++;
