  ...
  while (tries < 5) {
    try {
      ...
      const { data: html } = await axios.post(
        ...
        { id: videoUrl, locale: "fr", tt: token },
        {
          headers: {
            "Content-Type": "application/json",
      ...
      const hdVideo = $result("#hd_download").attr("data-directurl")
        ? "https://ssstik.io" + $result("#hd_download").attr("data-directurl")
        : null;

      const noWatermark = $result("a.download_link.without_watermark").attr("href") || null;
      const mp3 = $result("a.download_link.music").attr("href") || null;

      const slides = [];
      $result("a.download_link.slide").each((_, el) => slides.push($result(el).attr("href")));
