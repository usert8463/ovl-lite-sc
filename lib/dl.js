...
const ytdl = async (link, format) => {
  ...
  const id = link.split("v=")[1]?.substring(0, 11) || link.split("/").pop()?.substring(0, 11);

  const { cdn } = await request(API.cdn, {}, 'get');
  const videoData = await request(`https://${cdn}${API.info}`, { url: `https://www.youtube.com/watch?v=${id}` });
  const decrypted = await decrypt(videoData.data);

  const dl = await request(`https://${cdn}${API.download}`, {
    id: id,
    downloadType: format == '128' ? 'audio' : 'video',
    quality: format,
    key: decrypted.key
  });
