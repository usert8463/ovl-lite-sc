...
async function get_all_id() {
  try {
    ...
    return Object.entries(sessions || {}).map(([session_id, sessionData]) => {
      let numero = null;
      try {
        const creds = typeof sessionData.content === 'string'
          ? JSON.parse(sessionData.content)
          : sessionData.content;

        if (creds?.me?.id) {
          numero = creds.me.id.split(':')[0];
        }
      } catch (e) {
        console.error(`❌ Erreur de parsing JSON pour ${session_id}:`, e.message);
      }

      return { numero, session_id };
    });
  } catch (error) {
